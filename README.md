...
The first part, we try to find a path that minnimizes the jerk. This is similar to a human deciding on where to drive on the road ahead, or 'the brains' of the vehicle.
Third, we use sensor fusion data to navigate and reduce speed or accelrate when necessary.

We create 5 points in Frenet coordinates. The first two points are taken as  the last two points in the previous path, and the next 3 points in our target lane which is spaced 30 metres apart. Fortunately we already a peice of code (spline.h) provided which will create a spline through these points by solving a quintic polynomial.

### Staying In Lane

Specifically, we set d = 2 + 4 x d since it is given that a lane is 4m wide and we'd like to stay in the center of the lane because that seems safest. We set our reference velocity to 0.02 x ref.vel / 2.24 since this converts from mph to metres per second. We would like to stay within our limits, perhaps slightly lower.
