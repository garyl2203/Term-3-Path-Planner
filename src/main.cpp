...
vector<double> lane_shift(vector<vector<double>> sensor_fusion, int lane, double car_s, double car_d, double ref_vel)
{
    ...
    for (int i=0; i<sensor_fusion.size(); i++)
    {
        ...
        else if (d < (2+4*lane-2))
        {
            ...
            if (gap > 100) {
                // if car is 100 units ahead of our car, so no worries (cost=1)
                cost_left += 1;
            } else if (gap > 50) {
                // if car is 50 units ahead of our car, ok to shift (cost=5)
                cost_left += 5;
            } else if (gap > 0) {
                // if car is very close to our car, no to shift (cost=100)
                cost_left += 100;
            } else if (gap < -100) {
                // if car is 100 units behind of our car, so be cautious, but still can shift (cost=2)
                cost_left += 2;
            } else if (gap < -50) {
                // if car is 50 units behind our car, still ok to shift (cost=5)
                cost_left += 5;
            } else if (gap < 0) {
                // if car is very close to our car, no to shift (cost=100)
                cost_left += 100;
        ...
        else if (d > (2+4*lane+2))
        {
            ...
            if (gap > 100) {
                ...
                // if car is 50 units behind of our car, still ok to shift (cost=5)
                cost_right += 5;
            } else if (gap < 0) {
                // if car is very close to our car, no to shift (cost=100)
                cost_right += 100;
